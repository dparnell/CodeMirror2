...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function updateCursor() {
      var head = sel.inverted ? sel.from : sel.to, lh = lineHeight(), cw = stringWidth('x');
      var x = charX(head.line, head.ch) + "px", y = (head.line - showingFrom) * lh + "px";
      inputDiv.style.top = (head.line * lh - scroller.scrollTop) + "px";
      inputDiv.style.left = (head.ch * cw - scroller.scrollLeft) + "px";
    ...
    function highlightWorker() {
      ...
      while (work.length) {
        ...
        for (var i = start, l = lines.length; i < l; ++i) {
          ...
          line.stateAfter = copyState(mode, state);
          if (compare) {
            if (oldState && !compare(hadState, state)) break;
          } else {
